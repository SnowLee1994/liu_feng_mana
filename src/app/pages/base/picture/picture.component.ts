...
export class PictureComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.breadCrumbItems = [{label: '基础资源'}, {label: '图片管理', active: true}];
  ...
  private _fetchData(pageNum: number, pageSize: number) {
    ...
    this.pictureService.findPicturesByPage(sendData).subscribe(res => {
      console.log(res);
        if (res.success) {
          this.customersData = res.data.list;
          this.collectionSize = res.data.total;
        } else {
          alert('获取图片列表失败:' + res.msg);
        }
      },
      error => {
  ...
  openEditModal(content: any, editId: string) {
    ...
    this.pictureService.findRoleById(sendData).subscribe(r => {
        if (r.success) {
          this.modalService.open(content, {centered: true});
  ...
  saveData() {
    if (this.addform.valid) {
      this.pictureService.addRole(this.addform.value).subscribe(r => {
          if (r.success) {
            this.addform.reset();
  ...
  saveUpdate() {
    if (this.editform.valid) {
      this.pictureService.updateRole(this.editform.value).subscribe(r => {
          if (r.success) {
            this.modalService.dismissAll();
            this._fetchData(this.pageNum, this.pageSize);
          } else {
    ...
    swalWithBootstrapButtons
      ...
      .then(result => {
        if (result.value) {
          ...
          this.pictureService.deleteRole(sendData).subscribe(r => {
              console.log(r);
              if (r.success) {
